...
export async function ingest(db: D1Database, p: Payload) {
  ...
  await db
    ...
    .bind(deviceId, gatewayId, gatewayTime, n(p.espnow_rssi), n(p.wifi_rssi), n(p.protocol_version))
    .run();
  ...
  if (type === "telemetry") {
    await db
      ...
      .bind(
        recordId, deviceId, gatewayId, s(p.session_id), gatewayTime, s(p.gnss_time), n(p.lat), n(p.lon),
        n(p.alt_m), n(p.fix), n(p.satellites), n(p.hdop), n(p.speed_mps), n(p.course_deg), n(p.gnss_age_ms),
        n(p.espnow_rssi), n(p.packets_received), n(p.packets_lost), s(p.wifi_ssid), n(p.wifi_rssi),
        b(p.ntrip_enabled), b(p.ntrip_connected), n(p.rtcm_age_ms), n(p.rtcm_bytes), n(p.protocol_version), raw,
      )
      .run();
    ...
    await db
      ...
      .bind(
        measurementId, recordId, deviceId, gatewayId, s(p.session_id), s(p.name) ?? "Unnamed point", n(p.trigger),
        gatewayTime, s(p.gnss_time), n(p.lat), n(p.lon), n(p.alt_m), n(p.fix), n(p.satellites), n(p.hdop),
        n(p.espnow_rssi), raw,
      )
      .run();
  } else if (type === "event") {
    ...
    await db
      ...
      .bind(
        eventId, recordId, deviceId, gatewayId, s(p.session_id), s(p.event_type) ?? "event", s(p.detail),
        gatewayTime, n(p.espnow_rssi), raw,
      )
      .run();
    if (p.event_type === "measurement_cancel" && s(p.detail)) {
      await db
        .prepare("UPDATE measurements SET cancelled=1, updated_at=CURRENT_TIMESTAMP WHERE measurement_id=?")
        .bind(s(p.detail))
        .run();
    }
    ...
    await db
      ...
      .bind(sessionId, `Session ${sessionId.slice(0, 6)}`, active, gatewayTime, active, gatewayTime, active)
      .run();
  } else {
